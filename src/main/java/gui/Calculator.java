	...
	public void handle(ActionEvent event) {
		...
		if(input.equals("C")) {
			...
			if(operator.equals("+")) {
				...
				this.displayText.setText(Double.toString(resultado));
			}
			...
			operator=input;
			this.displayText.setText(numeros[0] + input);
			
		}
		else {
			if (numeros[0] != 0.0) {
				numeros[1] = Double.parseDouble(input);
			}
			else {
				numeros[0] = Double.parseDouble(input);
			}
			
			this.displayText.setText(this.displayText.getText()+input);
		}
